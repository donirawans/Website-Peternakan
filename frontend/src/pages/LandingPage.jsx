...
import { getLandingSettings, DEFAULT_FEATURES } from '../utils/landingSettings';
  ...
  const loadFarmSettings = useCallback(async () => {
    try {
      ...
      if (response.status === 200 && response.data) {
        ...
        if (response.data?.landing && typeof response.data.landing === 'object') {
          setLandingConfig(getLandingSettings(response.data.landing));
        }
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-surface antialiased selection:bg-primary/20 selection:text-primary">
      ...
      <main className="flex-grow pt-[84px]">
        {/* Hero Section */}
        <section className="relative min-h-[calc(100vh-84px)] flex items-center py-6 px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto overflow-hidden">
          <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-stretch">
            ...
            <div className="lg:col-span-6 flex flex-col justify-between gap-6 z-10">
              <div>
                <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#EBF1F9] border border-[#D9E3F0] text-slate-700 text-xs font-medium w-fit mb-4">
                  ...
                  <span className="text-slate-800 font-semibold tracking-wide">{landingConfig.heroBadge}</span>
                </div>

                <h1
                  className="font-display-lg text-display-lg md:text-[56px] md:leading-[64px] text-on-surface mb-4"
              ...
              <div className="flex flex-wrap items-center gap-4 mt-8 pt-2">
                <a
                  ...
                  className="inline-flex items-center gap-2 px-6 py-3.5 rounded-full bg-emerald-800 hover:bg-emerald-900 text-white font-semibold text-sm shadow-md transition-all"
                >
                ...
                <a
                  ...
                  className="inline-flex items-center gap-2 px-6 py-3.5 rounded-full bg-[#EBF1F9] hover:bg-slate-200 text-slate-800 font-semibold text-sm transition-all"
                >
            ...
            <div className="lg:col-span-6 relative w-full h-full min-h-[380px] lg:min-h-0 rounded-3xl overflow-hidden shadow-sm bg-slate-100 z-10 group">
        ...
        <section id="katalog" className="scroll-mt-24 max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop pt-4 pb-16">
          <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-6">
            <div>
              <h2 className="font-headline-lg text-headline-lg text-on-surface mb-2">Katalog Stok Sapi</h2>
              <p className="font-body-md text-body-md text-on-surface-variant">
                Update stok terbaru langsung dari kandang kami.
              </p>
            </div>
            {/* Category Tabs */}
            <div className="hidden md:flex gap-2 p-1 bg-surface-container rounded-xl">
              {CATEGORIES.map((cat) => (
                <button
                  key={cat}
        ...
        <section className="w-full bg-white py-24 scroll-mt-24" id="tentang">
          <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop">
            <div className="text-center max-w-2xl mx-auto mb-16">
              <h2 className="font-headline-lg text-headline-lg text-on-surface mb-4">{landingConfig.aboutTitle}</h2>
              <p className="font-body-md text-body-md text-on-surface-variant">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {(landingConfig.features && landingConfig.features.length ? landingConfig.features : DEFAULT_FEATURES).map((pillar) => (
                <div key={pillar.icon + pillar.title} className="bg-surface p-8 rounded-2xl border border-outline-variant/20 ambient-shadow text-center flex flex-col items-center">
                  <div className="w-16 h-16 bg-[#2D6A4F]/10 text-[#2D6A4F] rounded-2xl flex items-center justify-center mb-6">
                    <span className="material-symbols-outlined text-[32px]">{pillar.icon}</span>
                  </div>
                  <h3 className="font-headline-md text-headline-md text-on-surface font-bold mb-3">{pillar.title}</h3>
        ...
        <section
          id="lokasi"
          className="w-full scroll-mt-20 min-h-[calc(100vh-80px)] flex items-center justify-center bg-white px-margin-mobile md:px-margin-desktop py-8"
        >
          <div className="w-full max-w-container-max">
          <div className="bg-surface rounded-2xl border border-outline-variant/30 ambient-shadow p-8 md:p-12">
